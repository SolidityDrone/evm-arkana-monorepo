...
contract HuffConfig {
    ...
    function creation_code(string memory file) public payable returns (bytes memory bytecode) {
        ...
        // Use full path to huffc since FFI doesn't have PATH set up
        // Get home directory from environment with fallback
        string memory homeDir;
        try vm.envString("HOME") returns (string memory home) {
            homeDir = home;
        } catch {
            // Fallback to default home directory if HOME is not set
            homeDir = "/home/drone";
        }
        cmds[0] = string.concat(homeDir, "/.huff/bin/huffc");
        ...
        // Note: We don't use vm.prank here because:
        // 1. When broadcasting (via vm.startBroadcast()), prank is not allowed
        // 2. The actual msg.sender will be used, which is correct for both test and broadcast scenarios
        // The deployer parameter is still available via with_deployer() if needed for specific test scenarios
