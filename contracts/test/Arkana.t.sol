...
import {IPoseidonHasher} from "../src/merkle/IPoseidonHasher.sol";
import {PoseidonHasher} from "../src/merkle/PoseidonHasher.sol";
import {Field} from "../src/crypto-utils/Field.sol";
...
contract ArkanaTest is Test {
    ...
    function setUp() public {
        ...
        // Deploy PoseidonHasher (poseidon-solidity) and Arkana
        ...
        arkana = new Arkana(verifiers, 500, SEPOLIA_AAVE_POOL, 5, 31_536_000, address(ph), SEPOLIA_MULTICALL3);
        hasher = arkana.poseidonHasher();
        ...
        // Deploy vault for testToken and register with Arkana (vault init is outside Arkana)
        address aTokenAddress = IPool(SEPOLIA_AAVE_POOL).getReserveData(testTokenAddress).aTokenAddress;
        require(aTokenAddress != address(0), "No Aave reserve for test token");
        string memory symbol = IERC20Metadata(testTokenAddress).symbol();
        ArkanaVault vault = new ArkanaVault(
            IERC20(aTokenAddress),
            arkana,
            testTokenAddress,
            IPool(SEPOLIA_AAVE_POOL),
            string(abi.encodePacked("Arkana Vault ", symbol)),
            "ARK"
        );
        vault.initialize();
        arkana.registerVault(testTokenAddress, address(vault));

        // Verify vault was registered
        require(arkana.tokenVaults(testTokenAddress) != address(0), "Vault not registered");
    }
    ...
    function _addLeafViaInitialize(uint256 nonceCommitment) internal returns (uint256 root, uint256 expectedLeaf) {
        ...
        expectedLeaf = hasher.hash_2(finalCommitment.x, finalCommitment.y);
    ...
    ///      The SNARK_SCALAR_FIELD check in LeanIMTPoseidon.insert() is a safety check.
    ...
    function test_PoseidonHasher() public {
        assertTrue(address(hasher) != address(0), "Hasher should not be zero address");
        assertEq(address(hasher), address(arkana.poseidonHasher()), "Hasher should match contract's hasher");
    }
    ...
    /*   /// @notice Test entry -> send flow using values from Noir integration test
      ...
      function test_EntryThenSend_Integration() public {
          ...
          uint256 noteStackLeaf = hasher.hash_2(uint256(sendPublicInputs[15]), uint256(sendPublicInputs[16]));
      ...
      function test_EntryThenWithdraw_Integration() public {
          ...
          uint256 newCommitmentLeaf = hasher.hash_2(uint256(withdrawPublicInputs[8]), uint256(withdrawPublicInputs[9]));
          assertTrue(arkana.hasLeaf(testTokenAddress, newCommitmentLeaf), "New commitment leaf should exist in tree");
          console.log("  New commitment leaf verified in tree");
          console.log("");
