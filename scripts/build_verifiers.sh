...
# Script to build and generate verifiers for all Arkana circuits
...
circuits=("entry" "send" "absorb" "deposit" "withdraw")
...
process_circuit() {
    ...
    # Convert circuit_name to proper case for verifier name (e.g., entry -> Entry)
    local verifier_name=$(echo "$circuit_name" | sed 's/\([a-z]\)/\U\1/')
