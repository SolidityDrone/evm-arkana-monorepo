 ...
 */
function encryptAES128(data: string, key: Uint8Array): AESEncryption {
 ...
 */
export function decryptOrder(encryptedOrder: EncryptedOrder, drandSignature: any): any {
...
async function main() {
    ...
    encryptedOrders.forEach((order, index) => {
        ...
        console.log(`  Deadline: ${new Date(parseInt(order.orderData.deadline?.toString() || '0') * 1000).toISOString()}`);
