...
export class CachedUltraHonkBackend extends UltraHonkBackend {
    ...
    private acirBytecode: string | Uint8Array;

    constructor(
        acirBytecode: string | Uint8Array,
    ...
    private createCacheKey(bytecode: string | Uint8Array, options: BackendOptions): string {
        const optionsStr = JSON.stringify(options);
        // Convert bytecode to string for cache key
        const bytecodeStr = typeof bytecode === 'string' 
            ? bytecode 
            : Array.from(bytecode.slice(0, 20)).map(b => b.toString(16).padStart(2, '0')).join('');
        return `backend_${bytecodeStr}_${optionsStr}`;
    }
    ...
    async generateProof(compressedWitness: Uint8Array, options?: any): Promise<any> {
        ...
        const bytecodeForCache = typeof this.acirBytecode === 'string' 
            ? this.acirBytecode 
            : (() => {
                if (!globalThis.Buffer) {
                    throw new Error('Buffer polyfill not available');
                }
                return globalThis.Buffer.from(this.acirBytecode).toString('base64');
            })();
