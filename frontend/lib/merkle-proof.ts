...
export async function generateMerkleProof(
    ...
    console.log(`🌳 Generating proof for leaf[${leafIndex}] in tree with ${leaves.length} leaves, depth=${treeDepth}`);
    ...
    console.log(`✅ Generated proof: ${proof.length} siblings (padded to ${expectedDepth})`);
    ...
    if (!isValid) {
        console.error(`❌ Generated proof does not verify against contract root!`);
        throw new Error(`Proof verification failed for leaf[${leafIndex}]`);
    }

    console.log(`✅ Proof verified against contract root`);
